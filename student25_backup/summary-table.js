...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    data.forEach(s => {
  console.log("학생 전체 데이터 예시:", s);  // 🔍 추가
      const grade = parseInt(s.grade);
      const cls = parseInt(s.class);
      const gender = s.gender?.trim();
      const rawStatus = s.status;
const status = String(rawStatus ?? '').trim();
console.log(`${s.name} (${grade}-${cls}) → raw: [${rawStatus}], trimmed: [${status}], type: ${typeof rawStatus}`);



      console.log(`${s.name} (${grade}-${cls}) → status: [${status}]`);
